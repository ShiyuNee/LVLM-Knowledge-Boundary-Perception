...
def output_completion(completion,stream,useprob):
    if stream:
        ...
        return res, logprobs
    if useprob:
        # logprobs =  [ prob.logprob for prob in completion.choices[0].logprobs.content]
...
def has_answer(answers, text, match_type="string"):
    ...
    text = _normalize_answer(unicodedata.normalize('NFD', str(text))) # pred_text
...
def deal_judge_not_correct(pred):
    if pred is None:
        return True
    if has_answer(["no", "not correct", "incorrect", "not factually correct", "cannot"], pred):
        return True
    return False
